...
public class ClanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!plugin.getEnabledWorlds().contains(p.getWorld().getName().toLowerCase())) {
                        ...
                        public void run() {
                            if (p != null) {
                                if (clan != null) {
                                    ...
                                    p.sendMessage(ChatColor.BLUE + "Members (" + mems.size() + "): "
                                            + mems.toString().replace("[", "").replace("]", ""));
                                    if (plugin.getConfig().getBoolean("show-clan-base")) {
                                        try {
                                            p.sendMessage(ChatColor.BLUE + "Base: " + ChatColor.GREEN
                                                    + new SerializableLocation(clan.getBase()).toReadableString());
                                        } catch (Exception e) {
